...
> Answer: Client-side routing is usually faster and has smoother transitions because instead of redownloading the entire application only the necessary elements are updated and altered as opposed to server-side routing where the entire page is redownloaded. 

- [ ] What does AJAX stand for?
> Answer: Asynchronous JavaScript and XML. AJAX can utilize HTML, CSS the DOM and JavaScript working together to display content.

- [ ] What are `controlled components` in React?
> Answer: Controlled components in React are components whose state is mutable and can be managed and changed through the use of the react hook useState().
